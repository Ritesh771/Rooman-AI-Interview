...
export async function POST(req: NextRequest) {
  try {
    ...
    const codingInterview: CodingInterview = {
      title: title || `${role} Coding Interview - ${difficulty}`,
      difficulty,
      experienceLevel,
      numberOfQuestions,
      role,
    ...
    return NextResponse.json(codingInterview, { status: 200 });
  } catch (error) {
    console.error("Error generating coding interview:", error);
    return NextResponse.json(
      { error: "Failed to generate coding interview" }, 
      { status: 500 }
    );
  }
}